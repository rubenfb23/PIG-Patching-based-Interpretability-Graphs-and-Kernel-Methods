"""Ultra-simple local transformer model for fast patching tests.

This module provides a tiny causal transformer implemented from scratch
in PyTorch. It is designed to be API-compatible with the core methods used
by the patching pipeline, so tests can run without downloading HuggingFace
models.
"""
...
class TinyTransformerConfig:
    """Configuration for the tiny transformer test model."""
...
class ToyHookedModel(nn.Module):
    ...
    Node semantics:
    - `res`: residual stream after each layer
    - `mlp`: MLP output before residual addition
    - `att`: per-head attention output before output projection
    """
