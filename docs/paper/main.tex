...
\textbf{Evaluation.} We use seeds $\{7, 42, 123\}$ and $n \in \{20, 100, 500\}$ prompt pairs per corruption per seed. For $n=100$ and $n=500$, graph representation results use an example-disjoint split: $80$ (or $400$) training examples generate $32$ bootstrap graphs per slice, and $20$ (or $100$) test examples generate $32$ independent bootstrap graphs. Bootstrap graphs are constructed by resampling $75\%$ of the slice examples (with replacement, minimum $3$ examples) and recomputing the full graph for each resample. This procedure prevents the test-set leakage that can occur when the same prompt contributes to both training and test bootstrap graphs. Non-graph raw patch-effect and top-$k$ node rows are reported as per-example diagnostic upper bounds, not as graph-bootstrap results. All normalisation uses training-set statistics only; per-example diagnostics use fold-local normalisation. We report mean accuracy $\pm$ standard deviation across the three seeds. We avoid confidence intervals because three seeds are insufficient for stable interval estimates.
...
Classification & Linear SVM, example-disjoint train/test for graph rows \\
\bottomrule
\end{tabular}
...
\midrule
\multicolumn{5}{l}{\textit{Per-example signal diagnostics and localization baselines}} \\
...
\bottomrule
\end{tabular}
\caption{Compression trade-offs, non-graph baselines, and null controls on IOI ($n=100$, seeds $\{7,42,123\}$). Raw patch-effect and top-$k$ node rows are per-example diagnostic upper bounds showing that classification signal is already present before graph construction; they are not graph-bootstrap results. Scalable graph compressions preserve much of this localized signal while reducing dimensionality. Null controls show that fixed-layout performance depends on preserving signed node/edge identities.}
\label{tab:baselines}
\end{table}
