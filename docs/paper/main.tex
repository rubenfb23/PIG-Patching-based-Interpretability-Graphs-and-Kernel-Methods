...
\begin{abstract}
Mechanistic interpretability seeks to identify the internal algorithms implemented by trained neural networks. Activation patching exposes rich node-level signals, but the resulting tensors are difficult to compare and scale directly. We formalise patching-based interpretability graphs (PIG), a pipeline that compresses activation-patching signals into sparse directed graph representations, one per task slice. We use slice classification as a probe of signal preservation, not as an end in itself, and compare fixed-layout graph features, WL subtree features, spectral descriptors, graphlet counts, hashed edge-slot features, and coarse bucket counts. On the indirect object identification task with GPT-2, under an example-disjoint graph split, fixed-layout signed graph features achieve $0.9896$ mean accuracy but require $N(N-1)$ coordinates. Among scalable graph compressions, hashed fixed-layout features achieve $0.9375$ accuracy with $1024$ dimensions, while coarse bucket counts achieve $0.9271$ accuracy with approximately $105$ features. Global shape summaries such as spectral and graphlet features perform much worse, indicating that overly aggressive compression discards localized mechanistic information. Strict test-time edge and weight shuffling reduce fixed-layout weighted performance to chance, supporting that the observed signal depends on preserving graph structure rather than only edge-count or weight-marginal statistics.
\end{abstract}
