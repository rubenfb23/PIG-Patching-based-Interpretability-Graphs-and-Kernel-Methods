...
def test_linear_kernel(feature_matrix) -> bool:
    ...
    print(
        f"  CV accuracy: {cv_results['accuracy_mean']:.4f} "
...
def test_learning_curve(feature_matrix) -> bool:
    ...
    X = feature_matrix.to_matrix()
    y = feature_matrix.slice_labels
    y_labels = np.array([str(label) for label in y])

    clf = ClassicalKernelClassifier(kernel="linear", random_state=42)

    # Use stratified shuffled folds and avoid very small training subsets
    cv_splitter = StratifiedKFold(n_splits=2, shuffle=True, random_state=42)
    _, class_counts = np.unique(y_labels, return_counts=True)
    n_classes = len(class_counts)
    min_train_size = n_classes * 3

    first_train_idx, _ = next(cv_splitter.split(X, y_labels))
    max_train_size = len(first_train_idx)
    candidate_sizes = np.array([0.5, 0.7, 0.9, 1.0])
    train_sizes = np.unique((candidate_sizes * max_train_size).astype(int))
    train_sizes = train_sizes[train_sizes >= min_train_size]
    if train_sizes.size == 0:
        train_sizes = np.array([max_train_size], dtype=int)

    lc = clf.learning_curve(X, y, train_sizes=train_sizes, cv=cv_splitter)
