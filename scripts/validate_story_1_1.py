...
import torch
from pig.model import HookedModel, ActivationCache
...
def test_model_setup() -> bool:
    ...
    total_count = sum(1 for _ in model.model.parameters())
...
def test_forward_pass_speed(model: HookedModel, num_passes: int = 1000) -> bool:
    ...
    print(f"  Projected time for 10,000 passes: {time_for_10k:.1f}s ({time_for_10k/60:.1f} min)")
    print(f"  Device: {model.device}")
    ...
    print(f"  {status} {'Reasonable' if reasonable else 'Slow'} for 10k passes on {model.device.upper()}")
...
def test_activation_capture(model: HookedModel) -> bool:
    ...
    for layer, token in sample_positions:
        ...
        print(f"  Position ({layer}, {token}): shape={act.shape}, mean={act.mean():.4f}")
...
def test_observable_delta(model: HookedModel) -> bool:
    ...
    if significant_positions:
        ...
        sorted_positions = sorted(significant_positions, key=lambda x: abs(x[2]), reverse=True)[:5]
