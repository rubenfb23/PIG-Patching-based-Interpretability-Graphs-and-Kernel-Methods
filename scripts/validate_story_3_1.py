...
def test_correlation_weights(graphs: dict) -> bool:
    ...
    for slice_label, graph in graphs.items():
        ...
        print(
            f"  Slice {slice_label}: weight range [{min_w:.3f}, {max_w:.3f}], "
            f"mean={mean_w:.3f}"
        )
...
def test_direction_constraint(graphs: dict) -> bool:
    ...
    for slice_label, graph in graphs.items():
        ...
        print(
            f"  Slice {slice_label}: {len(graph.edges)} edges, "
...
def test_topk_sparsification(graphs: dict, k: int) -> bool:
    ...
    for slice_label, graph in graphs.items():
        ...
        print(
            f"  Slice {slice_label}: max_out_degree={max_out}, "
...
def test_edge_budget(graphs: dict, k: int) -> bool:
    ...
    for slice_label, graph in graphs.items():
        ...
        print(
            f"  Slice {slice_label}: {actual}/{max_budget} edges "
...
def main():
    ...
    print(
        f"\n  Dataset: {len(dataset)} examples, " f"{len(dataset.get_slices())} slices"
    )

    # Build graphs
    k = 5
    print(f"\n  Building graphs with k={k}...")
    builder = create_graph_builder(k=k, enforce_direction=True)
